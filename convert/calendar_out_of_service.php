...
class event_canceletion extends WebDriverTestCase {
    ...
    // clickElement
    $this->byCssSelector("#layout .popover.new_event .new-blocked-dates")->click();

      /*  $this->waitUntil(function() use ($test) {
            try {
            $boolean = ($test->byCssSelector("#layout #block-dates-modal[aria-hidden='false']") instanceof \PHPUnit_Extensions_Selenium2TestCase_Element);
        } catch (\Exception $e) {
            $boolean = false;
        }
            return $boolean === true ?: null;
        ...
        $this->byXPath('//*[@id="891-2"]/td[6]/div/div/div/div')->click();

        $this->waitUntil(function() use ($test) {
            try {
                $boolean = ($test->byCssSelector("#layout .popover.blocked_dates") instanceof \PHPUnit_Extensions_Selenium2TestCase_Element);
            } catch (\Exception $e) {
                $boolean = false;
            }
            return $boolean === true ?: null;
        ...
        // clickElement
        $this->byCssSelector("#layout .popover.blocked_dates .blocked-clear")->click();
    // clickElement
